...
void FPhysicsAssetEditorEditMode::Tick(FEditorViewportClient* ViewportClient, float DeltaTime)
{
	if (SharedData->bRunningSimulation)
	{
		...
		Setting->WorldGravityV = SharedData->bNoGravitySimulation ? FVector::ZeroVector : (SharedData->EditorOptions->bUseGravityOverride ? SharedData->EditorOptions->GravityOverrideV : (UPhysicsSettings::Get()->DefaultGravityV * SharedData->EditorOptions->GravScale));
