...
bool UGameplayStatics::BlueprintSuggestProjectileVelocity(const UObject* WorldContextObject, FVector& OutTossVelocity, FVector StartLocation, FVector EndLocation, float LaunchSpeed, FVector OverrideGravityV, ESuggestProjVelocityTraceOption::Type TraceOption, float CollisionRadius, bool bFavorHighArc, bool bDrawDebug)
{
	// simple pass-through to the C++ interface
	return UGameplayStatics::SuggestProjectileVelocity(WorldContextObject, OutTossVelocity, StartLocation, EndLocation, LaunchSpeed, bFavorHighArc, CollisionRadius, OverrideGravityV, TraceOption, FCollisionResponseParams::DefaultResponseParam, TArray<AActor*>(), bDrawDebug);
}
...
//Changed by Blake Richards
bool UGameplayStatics::SuggestProjectileVelocity(const UObject* WorldContextObject, FVector& OutTossVelocity, FVector Start, FVector End, float TossSpeed, bool bFavorHighArc, float CollisionRadius, FVector OverrideGravityV, ESuggestProjVelocityTraceOption::Type TraceOption, const FCollisionResponseParams& ResponseParam, const TArray<AActor*>& ActorsToIgnore, bool bDrawDebug)
{
	...
	//Changed by Blake Richards.
	const FVector GravityV = FMath::IsNearlyEqual(OverrideGravityV.Size(), 0.0f) ? -World->GetGravityV() : -OverrideGravityV;

	// v^4 - g*(g*x^2 + 2*y*v^2)
	const float InsideTheSqrt = FMath::Square(TossSpeedSq) - GravityV.Size() * ( (GravityV.Size() * FMath::Square(DeltaXY)) + (2.f * DeltaZ * TossSpeedSq) );
	...
	const float TanSolutionAngleA = (TossSpeedSq + SqrtPart) / (GravityV.Size() * DeltaXY);
	// this is the tangent of the firing angle for the second (-) solution
	const float TanSolutionAngleB = (TossSpeedSq - SqrtPart) / (GravityV.Size() * DeltaXY);
		...
		bFoundAValidSolution = true;
	 	...
	 	if (bDrawDebug)
	 	{
	 		...
	 		for ( float Step=0.f; Step<1.f; Step+=StepSize )
	 		{
	 			...
	 			// d = vt + .5 a t^2
	 			//Changed by Blake Richards
				const FVector TraceEnd = Start + OutTossVelocity*TimeInFlight + (-GravityV * FMath::Square(TimeInFlight) - CollisionRadius);
	...
	else
	{
		...
		for (int32 CurrentSolutionIdx=0; (CurrentSolutionIdx<2); ++CurrentSolutionIdx)
		{
			...
			for ( float Step=0.f; Step<1.f; Step+=StepSize )
			{
				...
				// d = vt + .5 a t^2
				//Changed by Blake Richards
				const FVector TraceEnd = Start + PrioritizedProjVelocities[CurrentSolutionIdx]*TimeInFlight + (-GravityV * FMath::Square(TimeInFlight) - CollisionRadius);
...
bool UGameplayStatics::PredictProjectilePath(const UObject* WorldContextObject, const FPredictProjectilePathParams& PredictParams, FPredictProjectilePathResult& PredictResult)
{
	...
	if (World && PredictParams.SimFrequency > UE_KINDA_SMALL_NUMBER)
	{
		...
		//Changed by Blake Richards
		const FVector GravityV = FMath::IsNearlyEqual(PredictParams.OverrideGravityV.Size(), 0.0f) ? World->GetGravityV() : PredictParams.OverrideGravityV;
		...
		while (CurrentTime < MaxSimTime)
		{
			...
			//Changed by Blake Richards
			CurrentVel = OldVelocity +  (GravityV * ActualStepDeltaTime);
			...
			if (bTracePath)
			{
				...
				if (bObjectHit || bChannelHit)
				{
					...
					const FVector VelocityAtHit = OldVelocity + (GravityV * HitTimeDelta);
	...
	FVector OverrideGravityV)
{
	...
	//Changed by Blake Richards
	Params.OverrideGravityV = OverrideGravityV;
...
//Changed by Blake Richards
bool UGameplayStatics::SuggestProjectileVelocity_CustomArc(const UObject* WorldContextObject, FVector& OutLaunchVelocity, FVector StartPos, FVector EndPos, FVector OverrideGravityV /*= 0*/, float ArcParam /*= 0.5f */)
{
	...
	if (World && StartToEndDist > UE_KINDA_SMALL_NUMBER)
	{
		...
		//Changed by Blake Richards
		float const NumeratorInsideSqrt = (GravityV.Size() * FMath::Square(Dx) * 0.5f);
