...
FVector AWorldSettings::GetGravityV() const
{
	if (!bWorldGravitySet)
	{
		// try to initialize cached value
		AWorldSettings* const MutableThis = const_cast<AWorldSettings*>(this);
		MutableThis->WorldGravityV = bGlobalGravitySet ? GlobalGravityV : UPhysicsSettings::Get()->DefaultGravityV;	//allows us to override DefaultGravityZ
	}

	return WorldGravityV;
}
...
#endif
//Changed by Blake Richards
void AWorldSettings::OnRep_WorldGravityV()
{
...
void AWorldSettings::GetLifetimeReplicatedProps( TArray< FLifetimeProperty > & OutLifetimeProps ) const
{
	...
	//Changed by Blake Richards
	DOREPLIFETIME( AWorldSettings, WorldGravityV );
...
void AWorldSettings::Serialize( FArchive& Ar )
{
	...
	{
		//before we had override flag we would use GlobalGravityZ != 0  Changed by Blake Richards
		if(GlobalGravityV.Size() != 0.0f)
		{
