...
void UCharacterMovementComponent::PerformAirControlForPathFollowing(FVector Direction, float ZDiff)
{
	...
	//Changed by Blake Richards, Needs work
	if ( CharacterOwner && Velocity.Z < 0.f && (ZDiff < 0.f || GetGravityV().Size() > 0.9f * GetWorld()->GetDefaultGravityV().Size()))
	{
		if ( ZDiff < 0.f )
		{
...
//Changed by Blake Richards
FVector UCharacterMovementComponent::GetGravityV() const
{
	return Super::GetGravityV() * GravityScale;
}
...
{
	//Changed by Blake Richards
	const float Gravity = GetGravityV().Size();
...
void UCharacterMovementComponent::ApplyRootMotionToVelocity(float deltaTime)
{
	...
	if( bAppliedRootMotion && AppliedVelocityDelta.Z != 0.f && IsMovingOnGround() )
	{
		...
		{
			// Default bounds - the amount of force gravity is applying this tick Change by Blake Richards
			LiftoffBound = FMath::Max(-GetGravityV().Size() * deltaTime, UE_SMALL_NUMBER);
		}
...
void UCharacterMovementComponent::PhysFalling(float deltaTime, int32 Iterations)
{
	...
	while( (remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations) )
	{
		...
		// Compute current gravity Changed by Blake Richards
...
void UCharacterMovementComponent::DisplayDebug(UCanvas* Canvas, const FDebugDisplayInfo& DebugDisplay, float& YL, float& YPos)
{
	...
	T = FString::Printf(TEXT("%s In physicsvolume %s on base %s component %s gravity %s"), *GetMovementName(), (PhysicsVolume ? *PhysicsVolume->GetName() : TEXT("None")),
		(BaseActor ? *BaseActor->GetName() : TEXT("None")), (BaseComponent ? *BaseComponent->GetName() : TEXT("None")), *GetGravityV().ToCompactString());
	DisplayDebugManager.DrawString(T);
}
...
void UCharacterMovementComponent::ApplyAccumulatedForces(float DeltaSeconds)
{
	if (PendingImpulseToApply.Z != 0.f || PendingForceToApply.Z != 0.f)
	{
		...
		//Changed by Blake Richards
		if ( IsMovingOnGround() && (PendingImpulseToApply.Z + (PendingForceToApply.Z * DeltaSeconds) + (GetGravityV().Size() * DeltaSeconds) > UE_SMALL_NUMBER))
		{
			SetMovementMode(MOVE_Falling);
		}
	}
...
void UCharacterMovementComponent::FillAsyncInput(const FVector& InputVector, FCharacterMovementComponentAsyncInput& AsyncInput)
{
	...
	AsyncInput.GravityV = GetGravityV();
