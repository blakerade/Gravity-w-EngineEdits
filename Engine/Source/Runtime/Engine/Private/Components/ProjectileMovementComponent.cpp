...
//Changed by Blake Richards
FVector UProjectileMovementComponent::GetGravityV() const
{
	// TODO: apply buoyancy if in water
	return ShouldApplyGravity() ? Super::GetGravityV() * ProjectileGravityScale : FVector::ZeroVector;
}
...
{
	return bForceSubStepping || (GetGravityV().Size() != 0.f) || (bIsHomingProjectile && HomingTargetComponent.IsValid());
}
