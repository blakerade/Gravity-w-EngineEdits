...
public:
	...
	*/
	 ...
	 * @param OverrideGravityV	Optional gravity override.  0 means "do not override". 
	 ...
	 */
	//Chnaged by Blake Richards
	UFUNCTION(BlueprintCallable, Category="Game", DisplayName="Suggest Projectile Velocity", meta=(WorldContext="WorldContextObject"))
	static bool BlueprintSuggestProjectileVelocity(const UObject* WorldContextObject, FVector& TossVelocity, FVector StartLocation, FVector EndLocation, float LaunchSpeed, FVector OverrideGravityV, ESuggestProjVelocityTraceOption::Type TraceOption, float CollisionRadius, bool bFavorHighArc, bool bDrawDebug);

	/** Native version, has more options than the Blueprint version. */ //Changed by Blake Richards
	static bool SuggestProjectileVelocity(const UObject* WorldContextObject, FVector& TossVelocity, FVector StartLocation, FVector EndLocation, float TossSpeed, bool bHighArc = false, float CollisionRadius = 0.f, FVector OverrideGravityV = FVector::ZeroVector, ESuggestProjVelocityTraceOption::Type TraceOption = ESuggestProjVelocityTraceOption::TraceFullPath, const FCollisionResponseParams& ResponseParam = FCollisionResponseParams::DefaultResponseParam, const TArray<AActor*>& ActorsToIgnore = TArray<AActor*>(), bool bDrawDebug = false);
	...
	* @param OverrideGravityV			Optional override of Gravity (if 0, uses WorldGravityZ)
	...
	* @param OverrideGravityV			Optional override of WorldGravityZ
	...
	*/
	...
	static bool SuggestProjectileVelocity_CustomArc(const UObject* WorldContextObject, FVector& OutLaunchVelocity, FVector StartPos, FVector EndPos, FVector OverrideGravityV = FVector::ZeroVector, float ArcParam = 0.5f);
