...
public:
	...
	// current gravity actually being used Changed by Blake Richards
	UPROPERTY(transient, ReplicatedUsing=OnRep_WorldGravityV)
	FVector WorldGravityV;

	//Changed by Blake Richards
	UFUNCTION()
	virtual void OnRep_WorldGravityV();

	// optional level specific gravity override set by level designer Changed by Blake Richards
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category=Physics, meta=(editcondition = "bGlobalGravitySet"))
	FVector GlobalGravityV;
...
public:
	 ...
	 */ //Changed by Blake Richards
	virtual FVector GetGravityV() const;
