...
void UChaosVehicleMovementComponent::UpdateState(float DeltaTime)
{
	// update input values
	AController* Controller = GetController();
	VehicleState.CaptureState(GetBodyInstance(), GetGravityV().Size(), DeltaTime);
...
void UChaosVehicleMovementComponent::CreateVehicle()
{
...

	{
		if (CanCreateVehicle())
		{
			...
			VehicleState.CaptureState(GetBodyInstance(), GetGravityV().Size(), 0.01667f);
	...
	if (CurAsyncInput)
	{
		if (const FBodyInstance* BodyInstance = GetBodyInstance())
		{
			if (auto Handle = BodyInstance->ActorHandle)
			{
				...
				AsyncInput->GravityZ = GetGravityV().Size();
