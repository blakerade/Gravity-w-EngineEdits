	...
	float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	for (TPair<FName, FPhysicsBodyModifier>& BodyModifierPair : Implementation->PhysicsBodyModifiers)
	{
		...
		if (BodyInstance->IsInstanceSimulatingPhysics())
		{
			...
			FVector GravityV = BodyInstance->GetPhysicsScene()->GetOwningWorld()->GetGravityV();
			FVector AppliedGravityV = BodyInstance->bEnableGravity ? GravityV : FVector::ZeroVector;
			FVector DesiredGravityV = GravityV * BodyModifier.GravityMultiplier;
			FVector GravityZToApply = DesiredGravityV - AppliedGravityV;
			BodyInstance->AddForce(GravityZToApply, true, true);
		}
	}
